...
const error = message => log(message, styles.red);
...
const getFilePath = request => {
	...
	if (!request.url.includes('.')) {
		...
		if (fs__default["default"].existsSync(testFilepath)) return testFilepath;
	}
...
const log = (message, color = styles.green) =>
...
const setRoot = () => {
	...
	return isRoot && fs__default["default"].existsSync(defaultRoot) ? defaultRoot : root;
};

const show404 = response => {
	fs__default["default"].readFile(`${options.root}/404.html`, (error, content) => {
...
const showFile = (response, content, contentType) => {
...
const start = (startOptions = {}) => {
	...
	const server = http__default["default"].createServer((request, response) => {
		...
		const extension = path__default["default"].extname(filePath).toLowerCase().slice(1);
		const contentType = mimeType(extension) || 'application/octet-stream';
		const isHtml = contentType == 'text/html';
		const encode = isHtml ? 'utf8' : null;

		fs__default["default"].readFile(filePath, encode, (error, content) => {
			if (error) {
